...
function withExtendedResolver(config, { tsconfig , isTsconfigPathsEnabled , isFastResolverEnabled , isExporting , isReactCanaryEnabled , isReactServerComponentsEnabled , getMetroBundler  }) {
    ...
    function getAssetRegistryPath() {
        ...
        if (config.transformer.assetRegistryPath && _path().default.isAbsolute(config.transformer.assetRegistryPath)) {
            _assetRegistryPath = _fs().default.realpathSync(config.transformer.assetRegistryPath);
            return _assetRegistryPath;
        }
        const assetRegistryPath = _fs().default.realpathSync(_path().default.resolve((0, _resolveFrom().default)(config.projectRoot, config.transformer.assetRegistryPath ?? "@react-native/assets-registry/registry.js")));
    ...
    const externals = [
        ...
        // Externals to speed up async split chunks by extern-ing common packages that appear in the root client chunk.
        {
            match: (context, moduleName, platform)=>{
                ...
                const isExternal = /^(styleq(\/.+)?|deprecated-react-native-prop-types|react-native-safe-area-context|invariant|nullthrows|memoize-one|@react-native\/assets-registry\/registry|react|react\/jsx-dev-runtime|scheduler|expo-modules-core|react-native|react-dom(\/.+)?|metro-runtime(\/.+)?)$/.test(moduleName) || /^react-native-web\/dist\/exports\/(Platform|NativeEventEmitter|StyleSheet|NativeModules|DeviceEventEmitter|Text|View)$/.test(moduleName) || // TODO: Add more
